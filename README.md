...
<img src="https://github.com/user-attachments/assets/67ad3f56-9b32-4328-9abe-0872a48d4c09" width="400" height="600" />
...
-   USB 디지털 현미경으로 사용자 요청이 들어오면 물 속 입자들을 관찰하고 영상 데이터 수집<br/>
...
-   검출된 미세 플라스틱 개수와 면적율 분석<br/>

④ 사용자 요청에 따른 알림 전송<br/>

-   사용자 요청 시 미세 플라스틱 검출 여부를 Telegram 챗봇으로 사용자에게 메시지 전송<br/>
-   미세 플라스틱 개수와 면적율에 따라서 각각 다른 메시지를 전송<br/>
    -   개수 <= 5, 면적율 <= 4: 정상<br/>
    -   개수 <= 10, 면적율 <= 10: 경고<br/>
    -   개수 > 10, 면적율 > 10: 필터 교체 필요<br/>
...
-   데이터셋에 들어있는 CSV 형식의 라벨 데이터를 YOLO 형식으로 변환
-   변환한 라벨 데이터를 이용해 YOLOv8n 모델을 학습
-   모델은 `microplastic.yaml` 데이터셋을 사용하여 학습

#### 3.3.2 시각화 구현

-   실시간으로 카메라에서 이미지를 캡처하고 YOLOv8n 모델을 통해 미세 플라스틱을 탐지
-   탐지된 객체는 경계 상자와 레이블, 신뢰도 점수와 함께 이미지에 표시

### 3.4. AI 모델 예측 결과<br/>

-   탐지 결과는 `detection_result.jpg` 파일로 저장되며, 탐지된 미세 플라스틱의 개수와 면적 비율이 반환되어 텔레그램 봇이 처리 가능
-   이미지
...
-   LBP 기반 모듈은 YOLOv8n 기반 모듈에 비해 성능과 정확도가 떨어짐<br/>
-   LBP 기반 모듈은 딥러닝 없이 히스토그램과 임계값 설정에 의존하는 규칙 기반 검출 방식을 사용하는 반면 YOLOv8n은 딥러닝 기술을 사용하고 실시간 처리에 특화되었기 때문에 YOLOv8n 기반 모듈이 유리함<br/>
